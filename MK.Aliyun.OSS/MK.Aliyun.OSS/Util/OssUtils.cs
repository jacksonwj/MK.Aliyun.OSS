    ...
    public static class OssUtils
    {
        ...
        public static string ComputeContentCrc64(Stream input, long length)
        {
            using (Crc64Stream crcStream = new Crc64Stream(input, null, length))
            {
                ...
                {
                    read = crcStream.Read(buffer, 0, buffer.Length);
        ...
        public static bool IsWebpageValid(string webpage)
        {
            ...
            return !string.IsNullOrEmpty(webpage) && webpage.EndsWith(pageSuffix) && webpage.Length > pageSuffix.Length;
        }
        ...
        public static bool IsLoggingPrefixValid(string loggingPrefix)
        {
            ...
            //return m.Success;
            return regex.Match(loggingPrefix).Success;
        }
        ...
        {
            return (partNumber.HasValue && partNumber > 0 && partNumber <= OssUtils.PartNumberUpperLimit);
        }
        ...
        internal static IAsyncResult BeginOperationHelper<TCommand>(TCommand cmd, AsyncCallback callback, Object state) where TCommand : OssCommand
        {
        ...
        internal static TResult EndOperationHelper<TResult>(IServiceClient serviceClient, IAsyncResult asyncResult)
        {
            ...
            if (retryableAsyncResult != null)
            {
                using (retryableAsyncResult)
                {
                    ...
                    var cmd = (OssCommand<TResult>)retryableAsyncResult.Context.Command;
        ...
        private static ServiceResponse EndOperationHelper(IServiceClient serviceClient, IAsyncResult asyncResult)
        {
            ...
            if (retryableAsyncResult == null)
            {
                var httpAsyncResult = asyncResult as ServiceClientImpl.HttpAsyncResult;
            ...
            if (retryableAsyncResult != null)
            {
                var httpAsyncResult = retryableAsyncResult.InnerAsyncResult as ServiceClientImpl.HttpAsyncResult;
        ...
        internal static Stream SetupProgressListeners(Stream originalStream, long progressUpdateInterval, object sender,
        ...
        internal static Stream SetupProgressListeners(Stream originalStream, long contentLength, long progressUpdateInterval, object sender,
                                                      EventHandler<StreamTransferProgressArgs> callback)
        {
        ...
        internal static Stream SetupProgressListeners(Stream originalStream, long contentLength, long totalBytesRead, long progressUpdateInterval,
        ...
        internal static Stream SetupDownloadProgressListeners(Stream originalStream, long contentLength, long totalBytesWrite, long progressUpdateInterval,
                                                              object sender, EventHandler<StreamTransferProgressArgs> callback)
        {
            var eventStream = new EventStream(originalStream, true);
        ...
        internal static void InvokeInBackground<T>(EventHandler<T> handler, T args, object sender) where T : EventArgs
        {
            ...
            foreach (var call in handler.GetInvocationList())
            {
                eventHandler = ((EventHandler<T>)call);
