...
#pragma warning disable 618, 3005
    ...
    public class CopyObjectRequest
    {
        ...
        /// If not, return 412 as HTTP code (precondition failed)
        /// </summary>        
        public IList<string> MatchingETagConstraints { get; } = new List<string>();
        ...
        /// If matches any, return 412 as HTTP code (precondition failed)
        /// </summary>       
        public IList<string> NonmatchingETagConstraints { get; } = new List<string>();
        ...
        internal void Populate(IDictionary<string, string> headers)
        {
            ...
            if (MatchingETagConstraints.Count > 0)
            {
                //headers.Add(OssHeaders.CopySourceIfMatch, OssUtils.JoinETag(_matchingETagConstraints));
                headers.Add(OssHeaders.CopySourceIfMatch, string.Join(", ", MatchingETagConstraints));
            }

            //if (_nonmatchingETagConstraints.Count > 0)
            if (NonmatchingETagConstraints.Count > 0)
            {
                //headers.Add(OssHeaders.CopySourceIfNoneMatch, OssUtils.JoinETag(_nonmatchingETagConstraints));
                headers.Add(OssHeaders.CopySourceIfNoneMatch, string.Join(", ", NonmatchingETagConstraints));
            }
